"""
FortiOS LOG - Log Anomaly

Log retrieval endpoint for log anomaly logs.
...
class Anomaly:
    ...
    Provides read-only access for FortiOS anomaly data.
    ...
    def __init__(self, client: "IHTTPClient", storage: str = "disk") -> None:
        ...
        self.raw = RawResource(client, "anomaly", storage)
        self._resource = LogResource(client, "anomaly", storage)
    ...
    def get(
        ...
        **kwargs: Any,
        ...
        1. Individual parameters: get(rows=100, filter='type==scan')
        2. Payload dict: get(payload_dict={'rows': 100, 'filter': 'type==scan'})
