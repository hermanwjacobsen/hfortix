"""
FortiOS LOG - Log File Filter

Log retrieval endpoint for log file filter logs.
...
Example Usage:
    ...
    >>> data = fgt.api.log.file_filter.get()
    >>>
...
class FileFilter:
    """
    Filefilter Operations.

    Provides read-only access for FortiOS filefilter data.
    ...
    def __init__(self, client: "IHTTPClient", storage: str = "disk") -> None:
        ...
        self.raw = RawResource(client, "file-filter", storage)
        self._resource = LogResource(client, "file-filter", storage)
