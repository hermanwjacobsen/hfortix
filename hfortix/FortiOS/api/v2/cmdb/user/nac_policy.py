"""
FortiOS CMDB - User NacPolicy

API Endpoints:
    GET    /user/nac-policy
    POST   /user/nac-policy
    GET    /user/nac-policy/{name}
    PUT    /user/nac-policy/{name}
    DELETE /user/nac-policy/{name}
"""
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            description: Description for the NAC policy matching pattern. (optional)
            category: Category of NAC policy. (optional)
            status: Enable/disable NAC policy. (optional)
            match_type: Match and retain the devices based on the type. (optional)
            match_period: Number of days the matched devices will be retained (0 - always retain) (optional)
            match_remove: Options to remove the matched override devices. (optional)
            ...
            severity: NAC policy matching devices vulnerability severity lists. (optional)
            switch_fortilink: FortiLink interface for which this NAC policy belongs to. (optional)
            switch_group: List of managed FortiSwitch groups on which NAC policy can be applied. (optional)
            switch_mac_policy: Switch MAC policy action to be applied on the matched NAC policy. (optional)
            firewall_address: Dynamic firewall address to associate MAC which match this policy. (optional)
            ssid_policy: SSID policy to be applied on the matched NAC policy. (optional)
            vdom: Virtual domain name, or False to skip. Handled by HTTPClient.
            raw_json: If True, return full API response with metadata. If False, return only results.
            **kwargs: Additional query parameters (filter, sort, start, count, format, etc.)
