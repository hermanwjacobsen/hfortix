"""
FortiOS CMDB - Switch-controller FortilinkSettings

API Endpoints:
    GET    /switch-controller/fortilink-settings
    POST   /switch-controller/fortilink-settings
    GET    /switch-controller/fortilink-settings/{name}
    PUT    /switch-controller/fortilink-settings/{name}
    DELETE /switch-controller/fortilink-settings/{name}
"""
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            fortilink: FortiLink interface to which this fortilink-setting belongs. (optional)
            inactive_timer: Time interval(minutes) to be included in the inactive devices expiry calculation (mac age-out + inactive-time + periodic scan interval). (optional)
            link_down_flush: Clear NAC and dynamic devices on switch ports on link down event. (optional)
            access_vlan_mode: Intra VLAN traffic behavior with loss of connection to the FortiGate. (optional)
