    ...
    def create(
        ...
        **kwargs
        ...
        if client_prefix_length is not None:
            data['client-prefix-length'] = client_prefix_length
        ...
        if icmp_session_quota is not None:
            data['icmp-session-quota'] = icmp_session_quota
        if name is not None:
            data['name'] = name
        if nat64 is not None:
            data['nat64'] = nat64
        if num_blocks_per_user is not None:
            data['num-blocks-per-user'] = num_blocks_per_user
        if pba_interim_log is not None:
            data['pba-interim-log'] = pba_interim_log
        if pba_timeout is not None:
            data['pba-timeout'] = pba_timeout
        if permit_any_host is not None:
            data['permit-any-host'] = permit_any_host
        if port_per_user is not None:
            data['port-per-user'] = port_per_user
        if privileged_port_use_pba is not None:
            data['privileged-port-use-pba'] = privileged_port_use_pba
        if source_endip is not None:
            data['source-endip'] = source_endip
        if source_prefix6 is not None:
            data['source-prefix6'] = source_prefix6
        if source_startip is not None:
            data['source-startip'] = source_startip
        ...
        if subnet_broadcast_in_ippool is not None:
            data['subnet-broadcast-in-ippool'] = subnet_broadcast_in_ippool
        if tcp_session_quota is not None:
            data['tcp-session-quota'] = tcp_session_quota
        if type is not None:
            data['type'] = type
        if udp_session_quota is not None:
            data['udp-session-quota'] = udp_session_quota
