"""
FortiOS CMDB - Diameter-filter Profile

API Endpoints:
    GET    /diameter-filter/profile
    POST   /diameter-filter/profile
    GET    /diameter-filter/profile/{name}
    PUT    /diameter-filter/profile/{name}
    DELETE /diameter-filter/profile/{name}
"""
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            monitor_all_messages: Enable/disable logging for all User Name and Result Code AVP messages. (optional)
            log_packet: Enable/disable packet log for triggered diameter settings. (optional)
            track_requests_answers: Enable/disable validation that each answer has a corresponding request. (optional)
            missing_request_action: Action to be taken for answers without corresponding request. (optional)
            protocol_version_invalid: Action to be taken for invalid protocol version. (optional)
            message_length_invalid: Action to be taken for invalid message length. (optional)
            request_error_flag_set: Action to be taken for request messages with error flag set. (optional)
            cmd_flags_reserve_set: Action to be taken for messages with cmd flag reserve bits set. (optional)
            command_code_invalid: Action to be taken for messages with invalid command code. (optional)
            command_code_range: Valid range for command codes (0-16777215). (optional)
            vdom: Virtual domain name, or False to skip. Handled by HTTPClient.
            raw_json: If True, return full API response with metadata. If False, return only results.
            **kwargs: Additional query parameters (filter, sort, start, count, format, etc.)
