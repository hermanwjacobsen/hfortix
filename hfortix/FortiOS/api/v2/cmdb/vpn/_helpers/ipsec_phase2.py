...
VALID_BODY_SELECTOR_MATCH = ["exact", "subset", "auto"]
VALID_BODY_PROPOSAL = [
    "null-md5",
    "null-sha1",
    "null-sha256",
...
VALID_BODY_SRC_ADDR_TYPE = ["subnet", "range", "ip", "name"]
VALID_BODY_DST_ADDR_TYPE = ["subnet", "range", "ip", "name"]
...
def validate_ipsec_phase2_post(
    ...
    if "selector-match" in payload:
        value = payload.get("selector-match")
...
def validate_ipsec_phase2_delete(
    name: str | None = None,
) -> tuple[bool, str | None]:
    """
