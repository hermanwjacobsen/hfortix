"""
FortiOS CMDB - Web-proxy ForwardServer

API Endpoints:
    GET    /web-proxy/forward-server
    POST   /web-proxy/forward-server
    GET    /web-proxy/forward-server/{name}
    PUT    /web-proxy/forward-server/{name}
    DELETE /web-proxy/forward-server/{name}
"""
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            healthcheck: Enable/disable forward server health checking. Attempts to connect through the remote forwarding server to a destination to verify that the forwarding server is operating normally. (optional)
            monitor: URL for forward server health check monitoring (default = www.google.com). (optional)
            server_down_option: Action to take when the forward server is found to be down: block sessions until the server is back up or pass sessions to their destination. (optional)
