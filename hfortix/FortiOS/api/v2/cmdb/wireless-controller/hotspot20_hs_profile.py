"""
FortiOS CMDB - Wireless-controller Hotspot20HsProfile

API Endpoints:
    GET    /wireless-controller.hotspot20/hs-profile
    POST   /wireless-controller.hotspot20/hs-profile
    GET    /wireless-controller.hotspot20/hs-profile/{name}
    PUT    /wireless-controller.hotspot20/hs-profile/{name}
    DELETE /wireless-controller.hotspot20/hs-profile/{name}
"""
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            release: Hotspot 2.0 Release number (1, 2, 3, default = 2). (optional)
            access_network_type: Access network type. (optional)
            access_network_internet: Enable/disable connectivity to the Internet. (optional)
            access_network_asra: Enable/disable additional step required for access (ASRA). (optional)
            access_network_esr: Enable/disable emergency services reachable (ESR). (optional)
            access_network_uesa: Enable/disable unauthenticated emergency service accessible (UESA). (optional)
            venue_group: Venue group. (optional)
            venue_type: Venue type. (optional)
            hessid: Homogeneous extended service set identifier (HESSID). (optional)
            proxy_arp: Enable/disable Proxy ARP. (optional)
            l2tif: Enable/disable Layer 2 traffic inspection and filtering. (optional)
            pame_bi: Enable/disable Pre-Association Message Exchange BSSID Independent (PAME-BI). (optional)
            ...
            dgaf: Enable/disable downstream group-addressed forwarding (DGAF). (optional)
            deauth_request_timeout: Deauthentication request timeout (in seconds). (optional)
            wnm_sleep_mode: Enable/disable wireless network management (WNM) sleep mode. (optional)
            bss_transition: Enable/disable basic service set (BSS) transition Support. (optional)
            ...
            wba_open_roaming: Enable/disable WBA open roaming support. (optional)
            wba_financial_clearing_provider: WBA ID of financial clearing provider. (optional)
            wba_data_clearing_provider: WBA ID of data clearing provider. (optional)
            wba_charging_currency: Three letter currency code. (optional)
            wba_charging_rate: Number of currency units per kilobyte. (optional)
            vdom: Virtual domain name, or False to skip. Handled by HTTPClient.
            raw_json: If True, return full API response with metadata. If False, return only results.
            **kwargs: Additional query parameters (filter, sort, start, count, format, etc.)
