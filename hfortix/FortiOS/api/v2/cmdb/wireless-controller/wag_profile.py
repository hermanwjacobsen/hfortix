"""
FortiOS CMDB - Wireless-controller WagProfile

API Endpoints:
    GET    /wireless-controller/wag-profile
    POST   /wireless-controller/wag-profile
    GET    /wireless-controller/wag-profile/{name}
    PUT    /wireless-controller/wag-profile/{name}
    DELETE /wireless-controller/wag-profile/{name}
"""
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            ping_interval: Interval between two tunnel monitoring echo packets (1 - 65535 sec, default = 1). (optional)
            ping_number: Number of the tunnel monitoring echo packets (1 - 65535, default = 5). (optional)
            return_packet_timeout: Window of time for the return packets from the tunnel's remote end (1 - 65535 sec, default = 160). (optional)
            dhcp_ip_addr: IP address of the monitoring DHCP request packet sent through the tunnel. (optional)
            vdom: Virtual domain name, or False to skip. Handled by HTTPClient.
            raw_json: If True, return full API response with metadata. If False, return only results.
            **kwargs: Additional query parameters (filter, sort, start, count, format, etc.)
