"""
FortiOS CMDB - Wireless-controller Global

API Endpoints:
    GET    /wireless-controller/global
    PUT    /wireless-controller/global
"""
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            location: Description of the location of the wireless controller. (optional)
            acd_process_count: Configure the number cw_acd daemons for multi-core CPU support (default = 0). (optional)
            wpad_process_count: Wpad daemon process count for multi-core CPU support. (optional)
            image_download: Enable/disable WTP image download at join time. (optional)
            rolling_wtp_upgrade: Enable/disable rolling WTP upgrade (default = disable). (optional)
            rolling_wtp_upgrade_threshold: Minimum signal level/threshold in dBm required for the managed WTP to be included in rolling WTP upgrade (-95 to -20, default = -80). (optional)
            max_retransmit: Maximum number of tunnel packet retransmissions (0 - 64, default = 3). (optional)
            control_message_offload: Configure CAPWAP control message data channel offload. (optional)
            data_ethernet_II: Configure the wireless controller to use Ethernet II or 802.3 frames with 802.3 data tunnel mode (default = enable). (optional)
            link_aggregation: Enable/disable calculating the CAPWAP transmit hash to load balance sessions to link aggregation nodes (default = disable). (optional)
            mesh_eth_type: Mesh Ethernet identifier included in backhaul packets (0 - 65535, default = 8755). (optional)
            fiapp_eth_type: Ethernet type for Fortinet Inter-Access Point Protocol (IAPP), or IEEE 802.11f, packets (0 - 65535, default = 5252). (optional)
            discovery_mc_addr: Multicast IP address for AP discovery (default = 244.0.1.140). (optional)
            discovery_mc_addr6: Multicast IPv6 address for AP discovery (default = FF02::18C). (optional)
            max_clients: Maximum number of clients that can connect simultaneously (default = 0, meaning no limitation). (optional)
            rogue_scan_mac_adjacency: Maximum numerical difference between an AP's Ethernet and wireless MAC values to match for rogue detection (0 - 31, default = 7). (optional)
            ipsec_base_ip: Base IP address for IPsec VPN tunnels between the access points and the wireless controller (default = 169.254.0.1). (optional)
            wtp_share: Enable/disable sharing of WTPs between VDOMs. (optional)
            tunnel_mode: Compatible/strict tunnel mode. (optional)
            nac_interval: Interval in seconds between two WiFi network access control (NAC) checks (10 - 600, default = 120). (optional)
            ap_log_server: Enable/disable configuring FortiGate to redirect wireless event log messages or FortiAPs to send UTM log messages to a syslog server (default = disable). (optional)
            ap_log_server_ip: IP address that FortiGate or FortiAPs send log messages to. (optional)
            ap_log_server_port: Port that FortiGate or FortiAPs send log messages to. (optional)
            max_sta_offline: Maximum number of station offline stored on the controller (default = 0). (optional)
            max_sta_offline_ip2mac: Maximum number of station offline ip2mac stored on the controller (default = 0). (optional)
            max_sta_cap: Maximum number of station cap stored on the controller (default = 0). (optional)
            max_sta_cap_wtp: Maximum number of station cap's wtp info stored on the controller (1 - 16, default = 8). (optional)
            max_rogue_ap: Maximum number of rogue APs stored on the controller (default = 0). (optional)
            max_rogue_ap_wtp: Maximum number of rogue AP's wtp info stored on the controller (1 - 16, default = 16). (optional)
            max_rogue_sta: Maximum number of rogue stations stored on the controller (default = 0). (optional)
            max_wids_entry: Maximum number of wids entries stored on the controller (default = 0). (optional)
            max_ble_device: Maximum number of BLE devices stored on the controller (default = 0). (optional)
            dfs_lab_test: Enable/disable DFS certificate lab test mode. (optional)
            vdom: Virtual domain name, or False to skip. Handled by HTTPClient.
            raw_json: If True, return full API response with metadata. If False, return only results.
            **kwargs: Additional query parameters (filter, sort, start, count, format, etc.)
