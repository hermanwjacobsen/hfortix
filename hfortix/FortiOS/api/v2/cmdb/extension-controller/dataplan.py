...
API Endpoints:
    GET    /extension-controller/dataplan
    POST   /extension-controller/dataplan
    GET    /extension-controller/dataplan/{name}
    PUT    /extension-controller/dataplan/{name}
    DELETE /extension-controller/dataplan/{name}
"""
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            signal_threshold: Signal threshold. Specify the range between 50 - 100, where 50/100 means -50/-100 dBm. (optional)
            signal_period: Signal period (600 to 18000 seconds). (optional)
            capacity: Capacity in MB (0 - 102400000). (optional)
            monthly_fee: Monthly fee of dataplan (0 - 100000, in local currency). (optional)
            ...
            private_network: Enable/disable dataplan private network support. (optional)
            vdom: Virtual domain name, or False to skip. Handled by HTTPClient.
            raw_json: If True, return full API response with metadata. If False, return only results.
            **kwargs: Additional query parameters (filter, sort, start, count, format, etc.)
