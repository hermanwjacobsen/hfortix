...
class ReverseConnector:
    ...
    def get(
        ...
        **kwargs: Any,
        ...
        # Build endpoint path
        if name:
            endpoint = f"/ztna/reverse-connector/{name}"
        else:
    ...
    def put(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            name: Reverse-Connector name (optional)
            status: Reverse-Connector status. (optional)
            address: Connector service edge adress(IP or FQDN). (optional)
            port: Port number that traffic uses to connect to connector service edge(0 - 65535;). (optional)
            health_check_interval: Health check interval in seconds (0 - 600, default = 60, 0 = disable). (optional)
            ssl_max_version: Highest TLS version acceptable from a server. (optional)
            certificate: The name of the certificate to use for SSL handshake. (optional)
            trusted_server_ca: Trusted Server CA certificate used by SSL connection. (optional)
            vdom: Virtual domain name, or False to skip. Handled by HTTPClient.
            raw_json: If True, return full API response with metadata. If False, return only results.
            **kwargs: Additional query parameters (filter, sort, start, count, format, etc.)
        ...
        if address is not None:
            data_payload["address"] = address
        if port is not None:
            data_payload["port"] = port
        if health_check_interval is not None:
            data_payload["health-check-interval"] = health_check_interval
        ...
        if trusted_server_ca is not None:
            data_payload["trusted-server-ca"] = trusted_server_ca
