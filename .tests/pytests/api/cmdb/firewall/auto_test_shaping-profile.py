...
class TestAutoShapingProfileGet:
    ...
    def auto_test_get_list_all(self):
        ...
        if len(result) > 0:
            ...
            print(f"   First item profile-name: {item.get('profile_name', 'N/A')}")
    ...
    def auto_test_get_specific_item(self):
        ...
        mkey_value = all_items[0]["profile_name"]
        
        # Get specific item
        result = endpoint.get(profile_name=mkey_value, response_mode="dict")
        
        # Since v0.5.33: querying by mkey returns single dict, not list
        assert isinstance(result, dict)
        assert result["profile_name"] == mkey_value
