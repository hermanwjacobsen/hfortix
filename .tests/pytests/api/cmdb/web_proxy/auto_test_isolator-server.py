"""
Auto-generated basic tests for cmdb.web_proxy/isolator_server

Generated from schema: /app/dev/classes/fortinet/schema/7.6.5/cmdb/web-proxy.isolator-server.json
Category: cmdb
Endpoint: /cmdb/web-proxy/isolator-server

These are BASIC automated tests. For comprehensive testing, create
manual tests in test_isolator-server.py
...
endpoint = fgt.api.cmdb.web_proxy.isolator_server


@pytest.mark.api_call
@pytest.mark.read_only
class TestAutoIsolatorServerGet:
    """Auto-generated GET operation tests."""
    
    def auto_test_get_list_all(self):
        """Test GET - list all isolator-server items."""
        try:
            result = endpoint.get()
        ...
        print(f"✅ Retrieved {len(result)} isolator-server items")
    ...
    def auto_test_get_specific_item(self):
        """Test GET - retrieve specific isolator-server by name."""
        ...
        if not all_items or len(all_items) == 0:
            pytest.skip("No existing isolator-server items to test with")
        ...
        print(f"✅ Retrieved isolator-server name={mkey_value}")
...
@pytest.mark.api_call
@pytest.mark.read_only
class TestAutoIsolatorServerExists:
    ...
    def auto_test_exists_method(self):
        ...
        if all_items and len(all_items) > 0:
            ...
            pytest.skip("No existing isolator-server items to test exists() with")


@pytest.mark.validator
@pytest.mark.parallel_safe
class TestAutoIsolatorServerValidators:
    """Auto-generated validator tests."""
    
    def auto_test_validator_import(self):
        """Test that validators can be imported."""
        try:
            from hfortix_fortios.api.v2.cmdb.web_proxy._helpers import isolator_server as validators
            print(f"✅ Successfully imported validators for isolator-server")
            
            # Check validator functions exist (they use HTTP method naming: post, put, get)
            assert hasattr(validators, "validate_web_proxy_isolator_server_post")
            assert hasattr(validators, "validate_web_proxy_isolator_server_put")
    ...
    def auto_test_validator_create_all_required(self):
        ...
        try:
            result = validators.validate_isolator_server_create(config)
...
@pytest.mark.validator
@pytest.mark.parallel_safe
class TestAutoIsolatorServerEnums:
...
TEST_ENDPOINT = "cmdb/web_proxy/isolator_server"
TEST_CATEGORY = "cmdb"
TEST_SCHEMA = "/app/dev/classes/fortinet/schema/7.6.5/cmdb/web-proxy.isolator-server.json"
