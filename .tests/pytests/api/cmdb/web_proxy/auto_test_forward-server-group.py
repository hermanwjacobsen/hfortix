...
class TestAutoForwardServerGroupGet:
    """Auto-generated GET operation tests."""
    
    def auto_test_get_list_all(self):
        """Test GET - list all forward-server-group items."""
        try:
        ...
        print(f"✅ Retrieved {len(result)} forward-server-group items")
    ...
    def auto_test_get_specific_item(self):
        """Test GET - retrieve specific forward-server-group by name."""
        ...
        if not all_items or len(all_items) == 0:
            pytest.skip("No existing forward-server-group items to test with")
        ...
        print(f"✅ Retrieved forward-server-group name={mkey_value}")
...
@pytest.mark.api_call
@pytest.mark.read_only
class TestAutoForwardServerGroupExists:
    ...
    def auto_test_exists_method(self):
        ...
        if all_items and len(all_items) > 0:
            ...
            pytest.skip("No existing forward-server-group items to test exists() with")
