"""
Auto-generated basic tests for cmdb.ips/custom

Generated from schema: /app/dev/classes/fortinet/schema/7.6.5/cmdb/ips.custom.json
Category: cmdb
...
endpoint = fgt.api.cmdb.ips.custom
...
class TestAutoCustomGet:
    ...
    def auto_test_get_list_all(self):
        ...
        if len(result) > 0:
            item = result[0]
            assert "tag" in item
            print(f"   First item tag: {item.get('tag', 'N/A')}")
    ...
    def auto_test_get_specific_item(self):
        ...
        mkey_value = all_items[0]["tag"]
        
        # Get specific item
        result = endpoint.get(tag=mkey_value, response_mode="dict")
        
        # Since v0.5.33: querying by mkey returns single dict, not list
        assert isinstance(result, dict)
        assert result["tag"] == mkey_value
    ...
    def auto_test_get_with_filters(self):
        ...
        try:
            result = endpoint.get(
                ...
                q_format="name|tag",  # Limit fields
                response_mode="dict",
            )
        except Exception as e:
            if any(code in str(e) for code in ["400", "404", "405", "424", "500", "503", "Bad Request", "Not Found", "Method Not Allowed", "Failed Dependency", "Internal Server Error", "Service Unavailable"]):
...
class TestAutoCustomEnums:
    ...
    def auto_test_enum_status(self):
        ...
        from hfortix_fortios.api.v2.cmdb.ips._helpers import custom as validators
...
TEST_ENDPOINT = "cmdb/ips/custom"
TEST_CATEGORY = "cmdb"
TEST_SCHEMA = "/app/dev/classes/fortinet/schema/7.6.5/cmdb/ips.custom.json"
