"""
Auto-generated basic tests for cmdb.videofilter/profile

Generated from schema: /app/dev/classes/fortinet/schema/7.6.5/cmdb/videofilter.profile.json
Category: cmdb
...
endpoint = fgt.api.cmdb.videofilter.profile
...
class TestAutoProfileValidators:
    ...
    def auto_test_validator_import(self):
        ...
        try:
            from hfortix_fortios.api.v2.cmdb.videofilter._helpers import profile as validators
            print(f"✅ Successfully imported validators for profile")
            
            # Check validator functions exist (they use HTTP method naming: post, put, get)
            assert hasattr(validators, "validate_videofilter_profile_post")
            assert hasattr(validators, "validate_videofilter_profile_put")
...
class TestAutoProfileEnums:
    ...
    def auto_test_enum_youtube(self):
        """Test enum field youtube validation."""
        from hfortix_fortios.api.v2.cmdb.videofilter._helpers import profile as validators
        ...
        for value in valid_values:
            config = {"youtube": value}
            # Validators should accept any valid enum value
            # Note: Full validation requires all required fields
            print(f"   Valid enum value: youtube={value}")
        
        print(f"✅ Enum field youtube has {len(valid_values)} valid values")
    def auto_test_enum_vimeo(self):
        """Test enum field vimeo validation."""
        from hfortix_fortios.api.v2.cmdb.videofilter._helpers import profile as validators
        ...
        for value in valid_values:
            ...
            # Validators should accept any valid enum value
            # Note: Full validation requires all required fields
            print(f"   Valid enum value: vimeo={value}")
        
        print(f"✅ Enum field vimeo has {len(valid_values)} valid values")
    def auto_test_enum_dailymotion(self):
        """Test enum field dailymotion validation."""
        from hfortix_fortios.api.v2.cmdb.videofilter._helpers import profile as validators
        ...
        # Test each valid value
        for value in valid_values:
            config = {"dailymotion": value}
            # Validators should accept any valid enum value
            # Note: Full validation requires all required fields
            print(f"   Valid enum value: dailymotion={value}")
        
        print(f"✅ Enum field dailymotion has {len(valid_values)} valid values")




# Metadata for test discovery
TEST_ENDPOINT = "cmdb/videofilter/profile"
TEST_CATEGORY = "cmdb"
TEST_SCHEMA = "/app/dev/classes/fortinet/schema/7.6.5/cmdb/videofilter.profile.json"
