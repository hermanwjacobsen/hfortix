...
class TestAutoOcspServerGet:
    """Auto-generated GET operation tests."""
    
    def auto_test_get_list_all(self):
        """Test GET - list all ocsp-server items."""
        try:
            result = endpoint.get()
        ...
        print(f"✅ Retrieved {len(result)} ocsp-server items")
    ...
    def auto_test_get_specific_item(self):
        """Test GET - retrieve specific ocsp-server by name."""
        ...
        if not all_items or len(all_items) == 0:
            pytest.skip("No existing ocsp-server items to test with")
        ...
        print(f"✅ Retrieved ocsp-server name={mkey_value}")
...
@pytest.mark.api_call
@pytest.mark.read_only
class TestAutoOcspServerExists:
    ...
    def auto_test_exists_method(self):
        ...
        if all_items and len(all_items) > 0:
            ...
            pytest.skip("No existing ocsp-server items to test exists() with")
