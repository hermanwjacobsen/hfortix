...
class TestAutoDdnsGet:
    ...
    def auto_test_get_list_all(self):
        ...
        if len(result) > 0:
            item = result[0]
            assert "ddnsid" in item
            print(f"   First item ddnsid: {item.get('ddnsid', 'N/A')}")
    ...
    def auto_test_get_specific_item(self):
        ...
        mkey_value = all_items[0]["ddnsid"]
        
        # Get specific item
        result = endpoint.get(ddnsid=mkey_value, response_mode="dict")
        
        # Since v0.5.33: querying by mkey returns single dict, not list
        assert isinstance(result, dict)
        assert result["ddnsid"] == mkey_value
