...
class TestAutoIpsUrlfilterDns6Get:
    ...
    def auto_test_get_list_all(self):
        ...
        if len(result) > 0:
            ...
            assert hasattr(item, "address6")
            mkey_value = getattr(item, "address6", "N/A")
            print(f"   First item address6: {mkey_value}")
    ...
    def auto_test_get_specific_item(self):
        ...
        mkey_value = getattr(all_items[0], "address6")
        
        # Get specific item
        result = endpoint.get(address6=mkey_value)
        
        # Since v0.5.33: querying by mkey returns single FortiObject, not list
        assert hasattr(result, "__dict__")  # FortiObject
        assert getattr(result, "address6") == mkey_value
