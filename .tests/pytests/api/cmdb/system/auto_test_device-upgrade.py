...
class TestAutoDeviceUpgradeGet:
    ...
    def auto_test_get_list_all(self):
        ...
        if len(result) > 0:
            ...
            print(f"   First item serial: {item.get('serial', 'N/A')}")
    ...
    def auto_test_get_specific_item(self):
        ...
        mkey_value = all_items[0]["serial"]
        
        # Get specific item
        result = endpoint.get(serial=mkey_value, response_mode="dict")
        
        # Since v0.5.33: querying by mkey returns single dict, not list
        assert isinstance(result, dict)
        assert result["serial"] == mkey_value
