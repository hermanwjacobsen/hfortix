...
Generated from schema: /app/dev/classes/fortinet/schema/7.6.5/cmdb/system.mac-address-table.json
Category: cmdb
...
class TestAutoMacAddressTableGet:
    ...
    def auto_test_get_list_all(self):
        ...
        if len(result) > 0:
            item = result[0]
            assert "mac" in item
            print(f"   First item mac: {item.get('mac', 'N/A')}")
    ...
    def auto_test_get_specific_item(self):
        ...
        mkey_value = all_items[0]["mac"]
        
        # Get specific item
        result = endpoint.get(mac=mkey_value, response_mode="dict")
        
        # Since v0.5.33: querying by mkey returns single dict, not list
        assert isinstance(result, dict)
        assert result["mac"] == mkey_value
    ...
    def auto_test_get_with_filters(self):
        ...
        try:
            result = endpoint.get(
                ...
                q_format="name|mac",  # Limit fields
                response_mode="dict",
            )
        except Exception as e:
            if any(code in str(e) for code in ["400", "404", "405", "424", "500", "503", "Bad Request", "Not Found", "Method Not Allowed", "Failed Dependency", "Internal Server Error", "Service Unavailable"]):
...
class TestAutoMacAddressTableValidators:
    ...
    def auto_test_validator_create_all_required(self):
        ...
        try:
            result = validators.validate_mac_address_table_create(config)
...
TEST_CATEGORY = "cmdb"
TEST_SCHEMA = "/app/dev/classes/fortinet/schema/7.6.5/cmdb/system.mac-address-table.json"
