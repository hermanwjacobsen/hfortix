...
class TestAutoServerGroupGet:
    """Auto-generated GET operation tests."""
    
    def auto_test_get_list_all(self):
        """Test GET - list all server-group items."""
        try:
            result = endpoint.get()
        ...
        print(f"✅ Retrieved {len(result)} server-group items")
    ...
    def auto_test_get_specific_item(self):
        """Test GET - retrieve specific server-group by name."""
        ...
        if not all_items or len(all_items) == 0:
            pytest.skip("No existing server-group items to test with")
        ...
        print(f"✅ Retrieved server-group name={mkey_value}")
...
@pytest.mark.api_call
@pytest.mark.read_only
class TestAutoServerGroupExists:
    ...
    def auto_test_exists_method(self):
        ...
        if all_items and len(all_items) > 0:
            ...
            pytest.skip("No existing server-group items to test exists() with")
