"""
Auto-generated basic tests for cmdb.antivirus/profile

Generated from schema: /app/dev/classes/fortinet/schema/7.6.5/cmdb/antivirus.profile.json
Category: cmdb
...
endpoint = fgt.api.cmdb.antivirus.profile
...
class TestAutoProfileValidators:
    ...
    def auto_test_validator_import(self):
        ...
        try:
            from hfortix_fortios.api.v2.cmdb.antivirus._helpers import profile as validators
            print(f"✅ Successfully imported validators for profile")
            
            # Check validator functions exist (they use HTTP method naming: post, put, get)
            assert hasattr(validators, "validate_antivirus_profile_post")
            assert hasattr(validators, "validate_antivirus_profile_put")
...
class TestAutoProfileEnums:
    ...
    def auto_test_enum_fortisandbox_mode(self):
        """Test enum field fortisandbox-mode validation."""
        from hfortix_fortios.api.v2.cmdb.antivirus._helpers import profile as validators
        
        valid_values = ['inline', 'analytics-suspicious', 'analytics-everything']
        
        # Test each valid value
        for value in valid_values:
            config = {"fortisandbox-mode": value}
            # Validators should accept any valid enum value
            # Note: Full validation requires all required fields
            print(f"   Valid enum value: fortisandbox-mode={value}")
        
        print(f"✅ Enum field fortisandbox-mode has {len(valid_values)} valid values")
    def auto_test_enum_analytics_db(self):
        """Test enum field analytics-db validation."""
        from hfortix_fortios.api.v2.cmdb.antivirus._helpers import profile as validators
        ...
        # Test each valid value
        for value in valid_values:
            config = {"analytics-db": value}
            # Validators should accept any valid enum value
            # Note: Full validation requires all required fields
            print(f"   Valid enum value: analytics-db={value}")
        
        print(f"✅ Enum field analytics-db has {len(valid_values)} valid values")




# Metadata for test discovery
TEST_ENDPOINT = "cmdb/antivirus/profile"
TEST_CATEGORY = "cmdb"
TEST_SCHEMA = "/app/dev/classes/fortinet/schema/7.6.5/cmdb/antivirus.profile.json"
